...
def compare_throws(user_throw, computer_throw_value):
    """
    Compare the user's and the computer's throws.
    Returns 'win', 'lose', or 'tie' (for the user).
    """
    if user_throw == computer_throw_value:
        return "tie"
    ...
    return "win" if wins[user_throw] == computer_throw_value else "lose"
...
def play_one_round():
    ...
    result = compare_throws(user, computer)
...
def finish_game(games_played, user_wins, computer_wins):
    ...
    if user_wins > computer_wins:
        print("Great job! You won the match!")
...
def start_game():
    ...
    print("---------------------------------")
    print("This is a Rock-Paper-Scissors championship match.")
